    ...
    <div class="page-wrap">
	  ...
	  <div class="nav-top">
		  <div class="nav-top-list">
			<ul class="clearfix" :style="{width:navWidth+'px'}">
				<li v-for="(sort,$index) in sortResource.topnav" :class="{active: sort.id === selectSortId}" :key="sort.id" @click="selSort(sort)">{{sort.name}}</li>
			</ul>
		  </div>
	  </div>
...
<style scoped>
  ...
  ul.sort-nav-list li.active:before {
    content: " ";
    display: inline-block;
    width: 3px;
    height: 20px;
